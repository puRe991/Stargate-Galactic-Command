...
namespace StargateGalacticCommand.Core.Services
{
    public class RegistrationService
    {
        ...
        public User CreateUserWithStartBase(IQueryable<User> users, IQueryable<Faction> factions, IQueryable<Planet> planets, string userName, string email, string password, int factionId)
        {
            ...
            var user = new User { UserName = userName.Trim(), Email = email.Trim().ToLowerInvariant(), PasswordHash = hash, PasswordSalt = salt, IsNpc = false, LastSeenAtUtc = null, FactionId = faction.Id, Faction = faction, ResearchLevels = new ResearchLevels(), CreatedAtUtc = DateTime.UtcNow };
